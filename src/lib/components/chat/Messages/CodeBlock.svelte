		...
		{:else}
				...
				<div class="flex items-center gap-0.5 shrink-0">
					{#if edit}
						<button
							...
							aria-pressed={isDiff ? editingDiff : editing}
							on:click={() => {
								if (isDiff) {
						...
						>
							{(isDiff ? editingDiff : editing) ? $i18n.t('Done') : $i18n.t('Edit')}
						</button>
					{/if}
					<button
			...
			>
				...
				{#if !collapsed}
					...
					{:else if edit && (isDiff ? editingDiff : editing)}
						<CodeEditor
							value={_code}
							{id}
							{lang}
