												...
												{#each toggleFilters as filter, filterIdx (filter.id)}
													<Tooltip content={filter?.description} placement="top">
														...
														>
															...
															{/if}
															<span
																class="hidden @xl:block whitespace-nowrap text-ellipsis leading-none normal-case"
																>{filter?.name}</span
															>
														</button>
													</Tooltip>
												...
												{#if showCodeInterpreterButton}
													<Tooltip content={$i18n.t('Execute code for analysis')} placement="top">
														<button
															...
															type="button"
															class="px-1.5 @xl:px-2.5 py-1.5 flex gap-1.5 items-center text-sm font-medium transition-colors duration-300 max-w-full overflow-hidden border {codeInterpreterEnabled
																? 'bg-gray-50 dark:bg-gray-400/10 border-gray-100  dark:border-gray-700 text-gray-600 dark:text-gray-400  '
																: 'bg-transparent border-transparent text-gray-600 dark:text-gray-300  hover:bg-gray-50 dark:hover:bg-gray-800 '} {($settings?.highContrastMode ??
															false)
																? 'm-1'
