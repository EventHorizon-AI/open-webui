										...
										{#if !embedded && prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														class=" bg-gray-100/50 text-white hover:bg-gray-100 dark:bg-gray-800/50 dark:text-black dark:hover:bg-gray-800 transition rounded-full p-[5px] self-center"
														type="button"
														on:click={async () => {
