...
def _complete_reasoning_item(reasoning_item: dict) -> None:
    """Mark an in-progress reasoning item as completed, filling ended_at/duration."""
                                ...
                                # Check for Responses API events (type field starts with "response.")
                                    ...
                                    if delta_tool_calls:
                                        ...
                                        if response_tool_calls:
                                            # The reasoning phase is over once the model starts emitting
                                            # tool calls, so close any still-open reasoning item.
                                            if output and output[-1].get('type') == 'reasoning':
                                                _complete_reasoning_item(output[-1])
                ...
                while tool_calls and (
                    ...
                    if ask_user_stage:
                        if ask_user_stage['error']:
                            ...
                            # Feed the validation error back to the model as the tool
                            # result and let it retry, instead of ending the turn.
                            try:
                                new_form_data = {
                                    **form_data,
                                    'model': model_id,
                                ...
                                log.exception('ask_user error continuation failed: %s', error_content)
                    ...
                    # Trim the trailing empty placeholder message from the frontend
                    # output too. The next tool-call continuation drops the same
                    # placeholder from prior_output before offsetting response
                    # output_index values, so keeping the client's item list in
                    # sync ensures the following round's reasoning/content deltas
                    # land on the correct items instead of the placeholder.
                    if (
                        frontend_output
                        and frontend_output[-1].get('type') == 'message'
                        and frontend_output[-1].get('status') == 'in_progress'
                    ):
                        msg_parts = frontend_output[-1].get('content', [])
                        if not msg_parts or (len(msg_parts) == 1 and not msg_parts[0].get('text', '').strip()):
                            frontend_output.pop()
